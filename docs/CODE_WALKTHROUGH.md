...
8. 创建 `AiAdvisor`。
9. 调用 AI 生成报告。
10. 把最近一次分析结果保存到 `st.session_state`。
11. 展示指标卡片、AI 报告、继续追问区、新闻链接、图表。
12. 生成 HTML 报告并提供下载。
...
初始化网页会话状态。Streamlit 每次点击按钮或输入内容都会重新运行脚本，所以需要用 `st.session_state` 保存最近一次分析结果。

它保存：

- `latest_result`：最近一次 `AnalysisResult`。
- `latest_ai_text`：最近一次 AI 报告。
- `latest_report_path`：最近一次 HTML 报告路径。
- `chat_history`：追问聊天记录。

### `_run_analysis()`

执行一次完整分析。它把原来写在 `main()` 里的分析流程拆出来，让网页结构更清楚。
...
- 调用 AI 生成报告。
- 可选生成 HTML 文件。
- 把结果写入 `st.session_state`。

### `_render_follow_up()`

渲染“继续追问”区域。

用户输入问题后，它会重新创建 `AiAdvisor`，然后调用：

```python
advisor.advise(result, user_question=question, report_depth=report_depth)
```

这里的关键是：追问不是重新拉行情，而是基于当前这份 `result` 数据底稿继续分析。

如果用户的问题里包含“走势图”“画图”“今天走势”“预测今日”等关键词，网页还会调用：

```python
build_today_prediction_html(result)
```

生成一张今日走势情景预测图。这个图来自本地数据和规则，不是 AI 直接画出来的图片。
...
10. 生成趋势标签、风险等级、操作建议。
11. 生成下一阶段预测。
12. 调用 `quant.py` 生成轻量量化上下文。
13. 打包成 `AnalysisResult`。

现在它还会生成 `market_data["level_context"]`，用于解决“支撑压力太远、不好交易”的问题：

- `short_term_supports` / `short_term_resistances`：短线支撑压力，来自日内高低点、上一收盘价、MA5、MA10 等。
- `swing_supports` / `swing_resistances`：波段支撑压力，来自 MA20、布林带、20 日高低点等。
- `extreme_supports` / `extreme_resistances`：极端风险位，来自 MA60、60 日高低点等。
- `long_risk_reward` / `short_risk_reward`：简单赔率估算，用来判断潜在收益是否值得承担止损风险。

这样做的原因是：只用 20 日低点和 60 日低点会让短线支撑太远，报告看起来不专业，也不好执行。
...
## 10. `stock_analyse/quant.py`

这个文件是轻量专业量化升级的第一步。它不做复杂机器学习，而是做可解释的因子和历史相似信号统计。
...
- `confidence`：量化置信度。
- `factors`：每个因子的分数和证据。
- `event_backtest`：历史相似信号回测。
- `strategy_backtest`：带入场、止损、止盈、交易成本和最大回撤的简化策略回测。
...
事件型回测。它会在历史 K 线中找出和当前同类的信号，然后统计未来 1 日、3 日、5 日的表现。
...
注意：这是轻量回测，不包含交易成本、滑点、仓位管理和样本外验证。

### `strategy_backtest()`

策略型回测。它比 `event_backtest()` 更接近真实交易，因为它会模拟交易动作：
...
这仍然是轻量回测，不等于专业交易系统。后续还需要加入真实滑点、样本外验证、参数稳定性和组合风险。

## 11. `stock_analyse/professional.py`
...
## 12. `stock_analyse/ai_advisor.py`
...
## 14. `stock_analyse/html_report.py`
