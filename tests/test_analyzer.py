...
class StockAnalyzerTest(unittest.TestCase):
    ...
    def test_analyze_generates_report_fields(self) -> None:
        ...
        self.assertIn("level_context", result.market_data)
        self.assertIsInstance(result.fundamental_data, dict)
        quant = result.fundamental_data.get("quant_context")
        self.assertIsInstance(quant, dict)
        self.assertTrue(quant.get("available"))
        self.assertIn("factors", quant)
        self.assertIn("factor_guide", quant)
        self.assertIn("metric_guide", quant)
        self.assertIn("event_backtest", quant)
        self.assertIn("strategy_backtest", quant)
        self.assertIn("available", quant["strategy_backtest"])
